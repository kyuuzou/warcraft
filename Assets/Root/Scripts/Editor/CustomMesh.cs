...
public class CreateMesh : ScriptableWizard {
    ...
    private void OnWizardCreate() {
        ...
        string planeAssetName = this.width + "x" + this.height + ".asset";
        Mesh m = new Mesh();
        ...
        float scaleY = this.height;
        for (float y = 0.0f; y < vCount2; y++) {
            for (float x = 0.0f; x < hCount2; x++) {
                vertices[index] = new Vector3(
                    x * scaleX - this.width / 2f - anchorOffset.x, y * scaleY - this.height / 2f - anchorOffset.y, 0.0f
                    );
