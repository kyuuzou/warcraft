...
public class CreatePlane : ScriptableWizard
{
    ...
    void OnWizardUpdate()
    {
        ...
        if (!targetAssetPath.Contains("Assets/Root/Meshes/")) {
            targetAssetPath = "Assets/Root/Meshes/";
            errorString = "Invalid asset path!";
            isValid = false;
        } else {
            errorString = "";
            isValid = true;
    ...
    void OnWizardCreate()
    {
        ...
        Mesh m = (Mesh)AssetDatabase.LoadAssetAtPath(targetAssetPath + planeAssetName, typeof(Mesh));
        ...
        if (m == null)
        {
            ...
            if (!targetAssetPath.EndsWith("/")) {
                targetAssetPath += "/";
            }
            
            string tempString = targetAssetPath.TrimEnd("/".ToCharArray());
            ...
            if (!System.IO.Directory.Exists(Application.dataPath + System.IO.Path.GetDirectoryName(targetAssetPath).TrimStart("Assets".ToCharArray()))) {
                AssetDatabase.CreateFolder(basePath, folderName);
            }
            
            string assetPathAndName = AssetDatabase.GenerateUniqueAssetPath(targetAssetPath + planeAssetName);
